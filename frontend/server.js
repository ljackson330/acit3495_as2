...
app.post("/login", async (req, res) => {
  ...
  try {
    console.log("Attempting to contact auth-service at http://auth-service:8001/login");
    ...
    if (error.response) {
      ...
      console.error("No response received from auth service. Request:", error.request);
    } else {
...
app.post("/api/insert-float", async (req, res) => {
  const token = req.headers["authorization"].split(" ")[1];
  try {
    const response = await axios.post("http://backend:8002/insert-float/", req.body, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
    ...
    res.status(error.response?.status || 500).json({ message: "Error inserting float", error: error.message });
  }
});
...
app.get("/api/get-stats", async (req, res) => {
  ...
  try {
    ...
    res.status(error.response?.status || 500).json({ message: "Error getting stats", error: error.message });
  }
});
...
app.get("/api/run-analytics", async (req, res) => {
  ...
  try {
    ...
    res.status(error.response?.status || 500).json({ message: "Error running analytics", error: error.message });
  }
});
